...
function SOR(objName, vertices, indexes, cylCounts, transMat, rotmat)
{
    this.objName = objName;
    this.vertices = vertices;
    this.indexes = indexes;
    this.cylCounts = cylCounts;
...
function saveFile(SOR) {
    ...
    // Convert cylCounts to Text
    for (var i = 0; i < SOR.cylCounts.length; i = i + 1) {
        text = text + "c " + SOR.cylCounts[i] + "\n";
    }
   // Convert transMat to Text
    for (var i = 0; i < SOR.transMat.length; i = i + 1) {
        text = text + "t " + SOR.transMat[i] + "\n";
    }
   // Convert rotmat to Text
    for (var i = 0; i < SOR.rotmat.length; i = i + 1) {
        text = text + "r " + SOR.rotmat[i] + "\n";
    }
    ...
    if (f) {
        ...
        r.onload = function(e) {
            ...
            while (1) {
                ...
                if (line.charAt(0) == "c") {
                    line = line.substr(2);
                    sorObject.cylCounts.push(parseFloat(line));
                }
                if (line.charAt(0) == "t") {
                    line = line.substr(2);
                    sorObject.transMat.push(parseFloat(line));
                }
                if (line.charAt(0) == "r") {
                    line = line.substr(2);
                    sorObject.rotmat.push(parseFloat(line));
                }
