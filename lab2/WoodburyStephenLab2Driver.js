...
var verticesWON;
var colors = [];       // Used to store Colors of verticies
...
function click(ev, gl, canvas, a_Position, a_Color) 
{
  ...
  if(RCPoints.length > 0 || colors.length != 0)
  {
...
function drawCylinderObjects(a_Position, gl, a_Color) 
{
  ...
  for(var i = 0; i < (shapes2.length-1); i+=2)
  {
    ...
    for(var j = 0; j < shapes2[i].length; j+=2)
    {
      ...
      if(counter < 11)
      {
        ...
        indices.push(j+1+(24*i));         //0  : 2  : 4  for i=0 -> 48 for i=2
        indices.push(j+(24*i)+2);         //1  : 3  : 5  for i=0 -> 49 for i=2
        indices.push((24*(i+1))+j+1);       //24 : 26 : 28 for i=0 -> 72 for i=2
        indices.push(j+(24*i)+2);         //1  : 3  : 5  for i=0 -> 49 for i=2
        indices.push((24*(i+1))+j+2);       //24 : 26 : 28 for i=0 -> 72 for i=2
  ...
  verticesWON = Float32Array.from(temp);
  ...
  generateColor(shapes2.length, temp.length)
  ...
  initArrayBuffer(gl, colorHolder, 3, gl.FLOAT, 'a_Color');
...
function generateColor(surfaceNum, vertexCt)
{
  tempCol = [];
  var templen;
  var nM, l, s;
  ...
  for(var i = 0; i < shapes2.length; i+=2)
  {
    for(var j = 0; j < 2; j++)
    {
      nM = new Vector3([(normals[(12*i)+23].elements[0]-normals[(12*i)+22].elements[0]), (normals[(12*i)+23].elements[1]-normals[(12*i)+22].elements[1]), (normals[(12*i)+23].elements[2]-normals[(12*i)+22].elements[2]) ]);
      nM.normalize();
      l = new Vector3([(1-normals[(12*i)+22].elements[0]), (1-normals[(12*i)+22].elements[1]), (1-normals[(12*i)+22].elements[2])]);
      l.normalize();
      s = ((nM.elements[0]*l.elements[0]) + (nM.elements[1]*l.elements[1]) + (nM.elements[2]*l.elements[2]));
      colors.push(new Vector3([0, s*2, 0]));
      for(var k = 1; k < (shapes2[i].length-2); k+=2)
      {
        nM = new Vector3([(normals[(12*i)+k].elements[0]-normals[(12*i)+(k-1)].elements[0]), (normals[(12*i)+k].elements[1]-normals[(12*i)+(k-1)].elements[1]), (normals[(12*i)+k].elements[2]-normals[(12*i)+(k-1)].elements[2]) ]);
        nM.normalize();
        l = new Vector3([(1-normals[(12*i)+(k-1)].elements[0]), (1-normals[(12*i)+(k-1)].elements[1]), (1-normals[(12*i)+(k-1)].elements[2])]);
        l.normalize();
  ...
  templen = vertexCt - tempCol.length;
...
function saveSOR()
{
  ...
  if (fileName != null) 
  {
    ...
    saveFile(new SOR(fileName, verticesWON, indexHolderWON));
  }
}
...
function readSOR()
{
  ...
  temp = Array.from(SORObj.vertices);
  indices = Array.from(SORObj.indexes);

  for(var i = 0; i<((temp.length)/72); i++)
  {
    var surface = [];
    for(var j = 0; j < (72); j+=3)
    {
      var ptHolder = new Vector3([temp[(i*72)+j],temp[(i*72)+j+1],temp[(i*72)+j+2]]);
  ...
  generateColor((temp.length  / 72), temp.length);
  ...
  var templen = vertices.length - tempCol.length;
  console.log('ver length: ' + vertices.length + ', tempClen: ' + tempCol.length + ', collen: ' + colors.length);
...
function generateNormal(i, j, e)
{
  ...
  if(e == 0)
  {
    v2 = new Vector3([ (shapes2[i+1][j+1].elements[0] - shapes2[i+1][j+2].elements[0]), (shapes2[i+1][j+1].elements[1] - shapes2[i+1][j+2].elements[1]), (shapes2[i+1][j+1].elements[2] - shapes2[i+1][j+2].elements[2])]);

  //Calculate our midway pt. between our 4 indices (Where our base pt. of our normal is going to go)
    pt1 = new Vector3([((shapes2[i][j+1].elements[0] + shapes2[i][j+2].elements[0] + shapes2[i+1][j+1].elements[0] + shapes2[i+1][j+2].elements[0]) / 4) , ((shapes2[i][j+1].elements[1] + shapes2[i][j+2].elements[1] + shapes2[i+1][j+1].elements[1] + shapes2[i+1][j+2].elements[1]) / 4) , ((shapes2[i][j+1].elements[2] + shapes2[i][j+2].elements[2] + shapes2[i+1][j+1].elements[2] + shapes2[i+1][j+2].elements[2]) / 4)]);
  }
  else
  {
